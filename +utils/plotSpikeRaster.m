function [plotH,gH,xPoints,yPoints] = plotSpikeRaster(spikes,varargin)
...
%
%   Output:
%       plotH - axes handle for raster plot
%       gH - handle to graphics object output
...
if islogical(spikes)
   ...
   switch plotType
      case 'horzline'
         ...
         xPoints = xPoints(:);
         yPoints = yPoints(:);
         gH = plot(xPoints,yPoints,'k',lineFormat{:});
      case 'vertline'
         %% Vertical Lines
         % Find the trial (yPoints) and timebin (xPoints) of each spike
         [trials,timebins] = find(spikes);
      ...
      case 'scatter'
         ...
         gH = plot(xPoints,yPoints,'.k',markerFormat{:});
   ...
   if strcmpi(plotType,'vertline') || strcmpi(plotType,'horzline')
      ...
      gH = plot(plotH,xPoints, yPoints, 'k', lineFormat{:});
