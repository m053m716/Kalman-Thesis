...
if exist(fname,'file')==0
   error('%s has not been created yet. Have you run cpl.optiSync and cpl.getButtonSync?',fname);
   
else
   ename = fullfile(DIR,[name EVENT_ID]);
   if exist(ename,'file')==0
      error('%s does not exist, although it should. Re-run cpl.optiSync and cpl.getButtonSync. Do overwrite when prompted.',ename);
   else
      load(ename,'tEvent');
   end
   
   behaviorData = cpl.readBehavior(fname,tEvent);
